// SPDX-FileCopyrightText: © Hewlett Packard Enterprise Development LP
// SPDX-License-Identifier: Apache-2.0
/**
 * Do not edit directly, this file was auto-generated.
 * Copyright Hewlett Packard Enterprise Development LP.
 */

export default {
  'hpe.breakpoint.large': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: ['WIDTH_HEIGHT'],
      },
    },
    $type: 'number',
    $value: '1440px',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: ['WIDTH_HEIGHT'],
        },
      },
      $type: 'number',
      $value: 1440,
    },
    name: 'hpe.breakpoint.large',
    attributes: {
      category: 'breakpoint',
      type: 'large',
    },
    path: ['breakpoint', 'large'],
    key: '{breakpoint.large}',
  },
  'hpe.breakpoint.medium': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: ['WIDTH_HEIGHT'],
      },
    },
    $type: 'number',
    $value: '1080px',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: ['WIDTH_HEIGHT'],
        },
      },
      $type: 'number',
      $value: 1080,
    },
    name: 'hpe.breakpoint.medium',
    attributes: {
      category: 'breakpoint',
      type: 'medium',
    },
    path: ['breakpoint', 'medium'],
    key: '{breakpoint.medium}',
  },
  'hpe.breakpoint.small': {
    $description:
      'The breakpoint where spacing and typography sizes shift down to accommodate smaller screen sizes.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: ['WIDTH_HEIGHT'],
      },
    },
    $type: 'number',
    $value: '768px',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description:
        'The breakpoint where spacing and typography sizes shift down to accommodate smaller screen sizes.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: ['WIDTH_HEIGHT'],
        },
      },
      $type: 'number',
      $value: 768,
    },
    name: 'hpe.breakpoint.small',
    attributes: {
      category: 'breakpoint',
      type: 'small',
    },
    path: ['breakpoint', 'small'],
    key: '{breakpoint.small}',
  },
  'hpe.breakpoint.xsmall': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: ['WIDTH_HEIGHT'],
      },
    },
    $type: 'number',
    $value: '576px',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: ['WIDTH_HEIGHT'],
        },
      },
      $type: 'number',
      $value: 576,
    },
    name: 'hpe.breakpoint.xsmall',
    attributes: {
      category: 'breakpoint',
      type: 'xsmall',
    },
    path: ['breakpoint', 'xsmall'],
    key: '{breakpoint.xsmall}',
  },
  'hpe.focusIndicator.boxShadow': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'shadow',
    $value: '0 0 0 2px #ffffff ',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'shadow',
      $value: [
        {
          blur: 0,
          color: '{color.focus.support}',
          offsetX: 0,
          offsetY: 0,
          spread: '{focusIndicator.outlineOffset}',
        },
      ],
    },
    name: 'hpe.focusIndicator.boxShadow',
    attributes: {
      category: 'focusIndicator',
      type: 'boxShadow',
    },
    path: ['focusIndicator', 'boxShadow'],
    key: '{focusIndicator.boxShadow}',
  },
  'hpe.focusIndicator.outline': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: [],
      },
    },
    $type: 'border',
    $value: {
      color: '#292d3a',
      style: 'solid',
      width: '2px',
    },
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: [],
        },
      },
      $type: 'border',
      $value: {
        color: '{color.focus.DEFAULT.REST}',
        style: 'solid',
        width: '{base.dimension.50}',
      },
    },
    name: 'hpe.focusIndicator.outline',
    attributes: {
      category: 'focusIndicator',
      type: 'outline',
    },
    path: ['focusIndicator', 'outline'],
    key: '{focusIndicator.outline}',
  },
  'hpe.focusIndicator.outlineOffset': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: [],
      },
    },
    $type: 'number',
    $value: '2px',
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: [],
        },
      },
      $type: 'number',
      $value: 2,
    },
    name: 'hpe.focusIndicator.outlineOffset',
    attributes: {
      category: 'focusIndicator',
      type: 'outlineOffset',
    },
    path: ['focusIndicator', 'outlineOffset'],
    key: '{focusIndicator.outlineOffset}',
  },
  'hpe.fontStack.primary': {
    $description: 'The main font stack for an application.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: true,
        scopes: ['FONT_FAMILY'],
      },
    },
    $type: 'fontFamily',
    $value: "'HPE Graphik', Arial, sans-serif",
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: 'The main font stack for an application.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: true,
          scopes: ['FONT_FAMILY'],
        },
      },
      $type: 'fontFamily',
      $value: "'HPE Graphik', Arial, sans-serif",
    },
    name: 'hpe.fontStack.primary',
    attributes: {
      category: 'fontStack',
      type: 'primary',
    },
    path: ['fontStack', 'primary'],
    key: '{fontStack.primary}',
  },
  'hpe.fontWeight.black': {
    $description: 'Black is rarely, if at all, used.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 900,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: 'Black is rarely, if at all, used.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 900,
    },
    name: 'hpe.fontWeight.black',
    attributes: {
      category: 'fontWeight',
      type: 'black',
    },
    path: ['fontWeight', 'black'],
    key: '{fontWeight.black}',
  },
  'hpe.fontWeight.bold': {
    $description:
      'Bold is used sparingly because it has reduced readability especially at smaller font sizes.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 700,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description:
        'Bold is used sparingly because it has reduced readability especially at smaller font sizes.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 700,
    },
    name: 'hpe.fontWeight.bold',
    attributes: {
      category: 'fontWeight',
      type: 'bold',
    },
    path: ['fontWeight', 'bold'],
    key: '{fontWeight.bold}',
  },
  'hpe.fontWeight.light': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 300,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 300,
    },
    name: 'hpe.fontWeight.light',
    attributes: {
      category: 'fontWeight',
      type: 'light',
    },
    path: ['fontWeight', 'light'],
    key: '{fontWeight.light}',
  },
  'hpe.fontWeight.medium': {
    $description:
      'The font-weight generally used when increased emphasis is desired.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 500,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description:
        'The font-weight generally used when increased emphasis is desired.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 500,
    },
    name: 'hpe.fontWeight.medium',
    attributes: {
      category: 'fontWeight',
      type: 'medium',
    },
    path: ['fontWeight', 'medium'],
    key: '{fontWeight.medium}',
  },
  'hpe.fontWeight.regular': {
    $description: 'The default font-weight for most text.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 400,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: 'The default font-weight for most text.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 400,
    },
    name: 'hpe.fontWeight.regular',
    attributes: {
      category: 'fontWeight',
      type: 'regular',
    },
    path: ['fontWeight', 'regular'],
    key: '{fontWeight.regular}',
  },
  'hpe.fontWeight.semibold': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 600,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 600,
    },
    name: 'hpe.fontWeight.semibold',
    attributes: {
      category: 'fontWeight',
      type: 'semibold',
    },
    path: ['fontWeight', 'semibold'],
    key: '{fontWeight.semibold}',
  },
  'hpe.fontWeight.thin': {
    $description: 'Thin is rarely, if at all, used.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FONT_STYLE'],
      },
    },
    $type: 'number',
    $value: 100,
    filePath: 'tokens/semantic/global.default.json',
    isSource: true,
    original: {
      $description: 'Thin is rarely, if at all, used.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FONT_STYLE'],
        },
      },
      $type: 'number',
      $value: 100,
    },
    name: 'hpe.fontWeight.thin',
    attributes: {
      category: 'fontWeight',
      type: 'thin',
    },
    path: ['fontWeight', 'thin'],
    key: '{fontWeight.thin}',
  },
};

// SPDX-FileCopyrightText: © Hewlett Packard Enterprise Development LP
// SPDX-License-Identifier: Apache-2.0
/**
 * Do not edit directly, this file was auto-generated.
 * Copyright Hewlett Packard Enterprise Development LP.
 */

export default {
  'hpe.color.background.active': {
    $description: 'Standard active state color',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.06)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Standard active state color',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity6}',
    },
    name: 'hpe.color.background.active',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'DEFAULT',
      subitem: 'active',
    },
    path: ['color', 'background', 'DEFAULT', 'active'],
    key: '{color.background.DEFAULT.active}',
  },
  'hpe.color.background.hover': {
    $description:
      'Generic hover state of components that have no fill in their resting state. For example, menu items.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.06)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Generic hover state of components that have no fill in their resting state. For example, menu items.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity6}',
    },
    name: 'hpe.color.background.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'DEFAULT',
      subitem: 'hover',
    },
    path: ['color', 'background', 'DEFAULT', 'hover'],
    key: '{color.background.DEFAULT.hover}',
  },
  'hpe.color.background.accent.blue.strong': {
    $description:
      'Use for strong blue accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#65aef9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong blue accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.200}',
    },
    name: 'hpe.color.background.accent.blue.strong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'blue',
      state: 'strong',
    },
    path: ['color', 'background', 'accent', 'blue', 'strong', 'REST'],
    key: '{color.background.accent.blue.strong.REST}',
  },
  'hpe.color.background.accent.blue.weak': {
    $description:
      'Use for subtle blue accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#1c3053',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for subtle blue accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.1000}',
    },
    name: 'hpe.color.background.accent.blue.weak',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'blue',
      state: 'weak',
    },
    path: ['color', 'background', 'accent', 'blue', 'weak', 'REST'],
    key: '{color.background.accent.blue.weak.REST}',
  },
  'hpe.color.background.accent.cyan.strong': {
    $description:
      'Use for strong cyan accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#00a4b3',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong cyan accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.cyan.400}',
    },
    name: 'hpe.color.background.accent.cyan.strong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'cyan',
      state: 'strong',
    },
    path: ['color', 'background', 'accent', 'cyan', 'strong', 'REST'],
    key: '{color.background.accent.cyan.strong.REST}',
  },
  'hpe.color.background.accent.cyan.weak': {
    $description:
      'Use for subtle cyan accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#163a3e',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for subtle cyan accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.cyan.1000}',
    },
    name: 'hpe.color.background.accent.cyan.weak',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'cyan',
      state: 'weak',
    },
    path: ['color', 'background', 'accent', 'cyan', 'weak', 'REST'],
    key: '{color.background.accent.cyan.weak.REST}',
  },
  'hpe.color.background.accent.purple.strong': {
    $description:
      'Use for strong purple accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#9b84fc',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong purple accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.purple.200}',
    },
    name: 'hpe.color.background.accent.purple.strong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'purple',
      state: 'strong',
    },
    path: ['color', 'background', 'accent', 'purple', 'strong', 'REST'],
    key: '{color.background.accent.purple.strong.REST}',
  },
  'hpe.color.background.accent.purple.weak': {
    $description:
      'Use for subtle purple accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#32275c',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for subtle purple accent backgrounds. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.purple.1000}',
    },
    name: 'hpe.color.background.accent.purple.weak',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'accent',
      subitem: 'purple',
      state: 'weak',
    },
    path: ['color', 'background', 'accent', 'purple', 'weak', 'REST'],
    key: '{color.background.accent.purple.weak.REST}',
  },
  'hpe.color.background.back': {
    $description:
      'Elevation level 0. Used for the overall page background. It creates a  backdrop for containers to sit on.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#1d1f27',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Elevation level 0. Used for the overall page background. It creates a  backdrop for containers to sit on.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1300}',
    },
    name: 'hpe.color.background.back',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'back',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'back', 'DEFAULT', 'REST'],
    key: '{color.background.back.DEFAULT.REST}',
  },
  'hpe.color.background.contrast': {
    $description:
      'Use to create a subtle differentiation between a component or container and the region it sits on.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.06)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use to create a subtle differentiation between a component or container and the region it sits on.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity6}',
    },
    name: 'hpe.color.background.contrast',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'contrast',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'contrast', 'DEFAULT', 'REST'],
    key: '{color.background.contrast.DEFAULT.REST}',
  },
  'hpe.color.background.contrast.hover': {
    $description:
      'Use to create a subtle differentiation between a component or container and the region it sits on.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.1)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use to create a subtle differentiation between a component or container and the region it sits on.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity10}',
    },
    name: 'hpe.color.background.contrast.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'contrast',
      subitem: 'DEFAULT',
      state: 'hover',
    },
    path: ['color', 'background', 'contrast', 'DEFAULT', 'hover'],
    key: '{color.background.contrast.DEFAULT.hover}',
  },
  'hpe.color.background.critical': {
    $description:
      'Use for backgrounds communicating errors or danger. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#552120',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for backgrounds communicating errors or danger. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.red.1000}',
    },
    name: 'hpe.color.background.critical',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'critical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'critical', 'DEFAULT', 'REST'],
    key: '{color.background.critical.DEFAULT.REST}',
  },
  'hpe.color.background.default': {
    $description: 'Default background color to be used on containers.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#1d1f27',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Default background color to be used on containers.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1300}',
    },
    name: 'hpe.color.background.default',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'default',
      subitem: 'REST',
    },
    path: ['color', 'background', 'default', 'REST'],
    key: '{color.background.default.REST}',
  },
  'hpe.color.background.disabled': {
    $description:
      'Disabled background color. Using disabled colors ensures that disabled components are consistently styled.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.07)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Disabled background color. Using disabled colors ensures that disabled components are consistently styled.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity7}',
    },
    name: 'hpe.color.background.disabled',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'disabled',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'disabled', 'DEFAULT', 'REST'],
    key: '{color.background.disabled.DEFAULT.REST}',
  },
  'hpe.color.background.floating': {
    $description:
      'Elevation level 2. The highest level of elevation for elements that sit above everything else. Example: drop downs, layers',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#22252e',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Elevation level 2. The highest level of elevation for elements that sit above everything else. Example: drop downs, layers',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1200}',
    },
    name: 'hpe.color.background.floating',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'floating',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'floating', 'DEFAULT', 'REST'],
    key: '{color.background.floating.DEFAULT.REST}',
  },
  'hpe.color.background.front': {
    $description:
      'Elevation level 1. Lowest level of elevation for containers/surfaces that sit directly on top of the page. Example: a container that houses a data table.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Elevation level 1. Lowest level of elevation for containers/surfaces that sit directly on top of the page. Example: a container that houses a data table.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.background.front',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'front',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'front', 'DEFAULT', 'REST'],
    key: '{color.background.front.DEFAULT.REST}',
  },
  'hpe.color.background.info': {
    $description:
      'Use for backgrounds communicating neutral information. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#2b3547',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for backgrounds communicating neutral information. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.1100}',
    },
    name: 'hpe.color.background.info',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'info',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'info', 'DEFAULT', 'REST'],
    key: '{color.background.info.DEFAULT.REST}',
  },
  'hpe.color.background.neutral.xstrong': {
    $description: 'Highest emphasis variant of neutral palette.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#f7f7f7',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Highest emphasis variant of neutral palette.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.50}',
    },
    name: 'hpe.color.background.neutral.xstrong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'neutral',
      subitem: 'xstrong',
      state: 'REST',
    },
    path: ['color', 'background', 'neutral', 'xstrong', 'REST'],
    key: '{color.background.neutral.xstrong.REST}',
  },
  'hpe.color.background.neutral.xstrong.hover': {
    $description: 'Hover variant of background-neutral-xstrong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Hover variant of background-neutral-xstrong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.200}',
    },
    name: 'hpe.color.background.neutral.xstrong.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'neutral',
      subitem: 'xstrong',
      state: 'hover',
    },
    path: ['color', 'background', 'neutral', 'xstrong', 'hover'],
    key: '{color.background.neutral.xstrong.hover}',
  },
  'hpe.color.background.ok': {
    $description:
      'Use for backgrounds communicating success. Status color. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#093d32',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for backgrounds communicating success. Status color. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.1000}',
    },
    name: 'hpe.color.background.ok',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'ok',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'ok', 'DEFAULT', 'REST'],
    key: '{color.background.ok.DEFAULT.REST}',
  },
  'hpe.color.background.primary.strong': {
    $description:
      'The strong variant of background-primary. Primary palette is derived from the brand color and used for high visual prominence.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'The strong variant of background-primary. Primary palette is derived from the brand color and used for high visual prominence.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.background.primary.strong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'primary',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'background', 'primary', 'strong', 'REST'],
    key: '{color.background.primary.strong.REST}',
  },
  'hpe.color.background.primary.strong.hover': {
    $description: 'Hover variant of background-primary.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#00e0af',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Hover variant of background-primary.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.500}',
    },
    name: 'hpe.color.background.primary.strong.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'primary',
      subitem: 'strong',
      state: 'hover',
    },
    path: ['color', 'background', 'primary', 'strong', 'hover'],
    key: '{color.background.primary.strong.hover}',
  },
  'hpe.color.background.primary.xstrong': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#66ebcf',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.450}',
    },
    name: 'hpe.color.background.primary.xstrong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'primary',
      subitem: 'xstrong',
      state: 'REST',
    },
    path: ['color', 'background', 'primary', 'xstrong', 'REST'],
    key: '{color.background.primary.xstrong.REST}',
  },
  'hpe.color.background.screenOverlay': {
    $description:
      'The background color for the overlay that sits behind modal layers.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(0, 0, 0, 0.48)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'The background color for the overlay that sits behind modal layers.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.black.opacity48}',
    },
    name: 'hpe.color.background.screenOverlay',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'screenOverlay',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'screenOverlay', 'DEFAULT', 'REST'],
    key: '{color.background.screenOverlay.DEFAULT.REST}',
  },
  'hpe.color.background.selected.primary': {
    $description: 'Lower emphasis variant of background-selected-strong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#093d32',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Lower emphasis variant of background-selected-strong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.1000}',
    },
    name: 'hpe.color.background.selected.primary',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'selected',
      subitem: 'primary',
      state: 'DEFAULT',
    },
    path: ['color', 'background', 'selected', 'primary', 'DEFAULT', 'REST'],
    key: '{color.background.selected.primary.DEFAULT.REST}',
  },
  'hpe.color.background.selected.primary.hover': {
    $description: 'Hover variant of background-selected-weak.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#074738',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Hover variant of background-selected-weak.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.900}',
    },
    name: 'hpe.color.background.selected.primary.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'selected',
      subitem: 'primary',
      state: 'DEFAULT',
    },
    path: ['color', 'background', 'selected', 'primary', 'DEFAULT', 'hover'],
    key: '{color.background.selected.primary.DEFAULT.hover}',
  },
  'hpe.color.background.selected.primary.strong': {
    $description:
      'Selected (or checked) color. Used for ‘selected’ state. Examples include, checked checkboxes, checked radios, on toggle, active tabs.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Selected (or checked) color. Used for ‘selected’ state. Examples include, checked checkboxes, checked radios, on toggle, active tabs.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.background.primary.strong.REST}',
    },
    name: 'hpe.color.background.selected.primary.strong',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'selected',
      subitem: 'primary',
      state: 'strong',
    },
    path: ['color', 'background', 'selected', 'primary', 'strong', 'REST'],
    key: '{color.background.selected.primary.strong.REST}',
  },
  'hpe.color.background.selected.primary.strong.hover': {
    $description:
      'Selected (or checked) color. Used for ‘selected’ state. Examples include, checked checkboxes, checked radios, on toggle, active tabs.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#00e0af',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Selected (or checked) color. Used for ‘selected’ state. Examples include, checked checkboxes, checked radios, on toggle, active tabs.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.background.primary.strong.hover}',
    },
    name: 'hpe.color.background.selected.primary.strong.hover',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'selected',
      subitem: 'primary',
      state: 'strong',
    },
    path: ['color', 'background', 'selected', 'primary', 'strong', 'hover'],
    key: '{color.background.selected.primary.strong.hover}',
  },
  'hpe.color.background.unknown': {
    $description:
      'Use for backgrounds communicating an unknown status. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.06)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for backgrounds communicating an unknown status. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity6}',
    },
    name: 'hpe.color.background.unknown',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'unknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'unknown', 'DEFAULT', 'REST'],
    key: '{color.background.unknown.DEFAULT.REST}',
  },
  'hpe.color.background.warning': {
    $description:
      'Use for backgrounds communicating warning or caution. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#3c361e',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for backgrounds communicating warning or caution. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.orange.1000}',
    },
    name: 'hpe.color.background.warning',
    attributes: {
      category: 'color',
      type: 'background',
      item: 'warning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'background', 'warning', 'DEFAULT', 'REST'],
    key: '{color.background.warning.DEFAULT.REST}',
  },
  'hpe.color.border.accent.blue.strong': {
    $description:
      'Use for strong blue accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#65aef9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong blue accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.200}',
    },
    name: 'hpe.color.border.accent.blue.strong',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'accent',
      subitem: 'blue',
      state: 'strong',
    },
    path: ['color', 'border', 'accent', 'blue', 'strong', 'REST'],
    key: '{color.border.accent.blue.strong.REST}',
  },
  'hpe.color.border.accent.cyan.strong': {
    $description:
      'Use for strong cyan accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#00a4b3',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong cyan accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.cyan.400}',
    },
    name: 'hpe.color.border.accent.cyan.strong',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'accent',
      subitem: 'cyan',
      state: 'strong',
    },
    path: ['color', 'border', 'accent', 'cyan', 'strong', 'REST'],
    key: '{color.border.accent.cyan.strong.REST}',
  },
  'hpe.color.border.accent.purple.strong': {
    $description:
      'Use for strong purple accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#9b84fc',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for strong purple accent borders. Should be used sparingly. The hue carries no fixed meaning. Keywords: accent, brand, decorative',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.purple.200}',
    },
    name: 'hpe.color.border.accent.purple.strong',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'accent',
      subitem: 'purple',
      state: 'strong',
    },
    path: ['color', 'border', 'accent', 'purple', 'strong', 'REST'],
    key: '{color.border.accent.purple.strong.REST}',
  },
  'hpe.color.border.critical': {
    $description:
      'Use for borders communicating errors or danger. Keywords: status, critical',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#fc6161',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for borders communicating errors or danger. Keywords: status, critical',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{color.foreground.critical.DEFAULT.REST}',
    },
    name: 'hpe.color.border.critical',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'critical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'critical', 'DEFAULT', 'REST'],
    key: '{color.border.critical.DEFAULT.REST}',
  },
  'hpe.color.border.default': {
    $description: 'Default border color.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.36)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Default border color.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity36}',
    },
    name: 'hpe.color.border.default',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'default',
      subitem: 'REST',
    },
    path: ['color', 'border', 'default', 'REST'],
    key: '{color.border.default.REST}',
  },
  'hpe.color.border.disabled': {
    $description:
      'Disabled color for borders. Using disabled colors ensures that disabled components are consistently styled.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.12)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Disabled color for borders. Using disabled colors ensures that disabled components are consistently styled.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity12}',
    },
    name: 'hpe.color.border.disabled',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'disabled',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'disabled', 'DEFAULT', 'REST'],
    key: '{color.border.disabled.DEFAULT.REST}',
  },
  'hpe.color.border.info': {
    $description:
      'Use for borders communicating neutral information. Keyword: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.36)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for borders communicating neutral information. Keyword: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.border.default.REST}',
    },
    name: 'hpe.color.border.info',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'info',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'info', 'DEFAULT', 'REST'],
    key: '{color.border.info.DEFAULT.REST}',
  },
  'hpe.color.border.ok': {
    $description: 'Use for borders communicating success. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.36)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Use for borders communicating success. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.border.default.REST}',
    },
    name: 'hpe.color.border.ok',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'ok',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'ok', 'DEFAULT', 'REST'],
    key: '{color.border.ok.DEFAULT.REST}',
  },
  'hpe.color.border.selected': {
    $description: 'Selected border color. Used for ‘selected’ state.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Selected border color. Used for ‘selected’ state.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.foreground.primary.DEFAULT.REST}',
    },
    name: 'hpe.color.border.selected',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'selected',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'selected', 'DEFAULT', 'REST'],
    key: '{color.border.selected.DEFAULT.REST}',
  },
  'hpe.color.border.strong': {
    $description: 'Emphasized variant of border color.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.72)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Emphasized variant of border color.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity72}',
    },
    name: 'hpe.color.border.strong',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'strong',
      subitem: 'REST',
    },
    path: ['color', 'border', 'strong', 'REST'],
    key: '{color.border.strong.REST}',
  },
  'hpe.color.border.unknown': {
    $description:
      'Use for borders communicating an unknown status. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.36)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for borders communicating an unknown status. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.border.default.REST}',
    },
    name: 'hpe.color.border.unknown',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'unknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'unknown', 'DEFAULT', 'REST'],
    key: '{color.border.unknown.DEFAULT.REST}',
  },
  'hpe.color.border.warning': {
    $description:
      'Use for borders communicating warning or caution. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.36)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for borders communicating warning or caution. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.border.default.REST}',
    },
    name: 'hpe.color.border.warning',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'warning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'border', 'warning', 'DEFAULT', 'REST'],
    key: '{color.border.warning.DEFAULT.REST}',
  },
  'hpe.color.border.weak': {
    $description: 'Subtle variant of border color for reduced visual emphasis.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.12)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Subtle variant of border color for reduced visual emphasis.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity12}',
    },
    name: 'hpe.color.border.weak',
    attributes: {
      category: 'color',
      type: 'border',
      item: 'weak',
      subitem: 'REST',
    },
    path: ['color', 'border', 'weak', 'REST'],
    key: '{color.border.weak.REST}',
  },
  'hpe.color.dataVis.categorical.10': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#65aef9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.200}',
    },
    name: 'hpe.color.dataVis.categorical.10',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '10',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '10', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.10.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.20': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#00e0af',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.400}',
    },
    name: 'hpe.color.dataVis.categorical.20',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '20',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '20', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.20.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.30': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#9b84fc',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.purple.200}',
    },
    name: 'hpe.color.dataVis.categorical.30',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '30',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '30', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.30.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.40': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#0070f8',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.500}',
    },
    name: 'hpe.color.dataVis.categorical.40',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '40',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '40', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.40.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.50': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#cc54a4',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.fuschia.300}',
    },
    name: 'hpe.color.dataVis.categorical.50',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '50',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '50', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.50.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.60': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#00dbe4',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.cyan.300}',
    },
    name: 'hpe.color.dataVis.categorical.60',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '60',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '60', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.60.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.70': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#c98ad4',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.plum.100}',
    },
    name: 'hpe.color.dataVis.categorical.70',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '70',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '70', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.70.DEFAULT.REST}',
  },
  'hpe.color.dataVis.categorical.80': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#e5786a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.coral.200}',
    },
    name: 'hpe.color.dataVis.categorical.80',
    attributes: {
      category: 'color',
      type: 'dataVis',
      item: 'categorical',
      subitem: '80',
      state: 'DEFAULT',
    },
    path: ['color', 'dataVis', 'categorical', '80', 'DEFAULT', 'REST'],
    key: '{color.dataVis.categorical.80.DEFAULT.REST}',
  },
  'hpe.color.decorative.blue': {
    $description:
      'Use for decorative purposes when color has no specific meaning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#0070f8',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for decorative purposes when color has no specific meaning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.500}',
    },
    name: 'hpe.color.decorative.blue',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'blue',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'blue', 'DEFAULT', 'REST'],
    key: '{color.decorative.blue.DEFAULT.REST}',
  },
  'hpe.color.decorative.brand': {
    $description: 'The brand color.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#01a982',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'The brand color.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.600}',
    },
    name: 'hpe.color.decorative.brand',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'brand',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'brand', 'DEFAULT', 'REST'],
    key: '{color.decorative.brand.DEFAULT.REST}',
  },
  'hpe.color.decorative.cyan': {
    $description:
      'Use for decorative purposes when color has no specific meaning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#00a4b3',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for decorative purposes when color has no specific meaning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.cyan.400}',
    },
    name: 'hpe.color.decorative.cyan',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'cyan',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'cyan', 'DEFAULT', 'REST'],
    key: '{color.decorative.cyan.DEFAULT.REST}',
  },
  'hpe.color.decorative.green': {
    $description:
      'Use for decorative purposes when color has no specific meaning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#068667',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for decorative purposes when color has no specific meaning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.700}',
    },
    name: 'hpe.color.decorative.green',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'green',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'green', 'DEFAULT', 'REST'],
    key: '{color.decorative.green.DEFAULT.REST}',
  },
  'hpe.color.decorative.neutral': {
    $description: 'Highest emphasis variant of neutral palette.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#b1b9be',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Highest emphasis variant of neutral palette.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.500}',
    },
    name: 'hpe.color.decorative.neutral',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'neutral',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'neutral', 'DEFAULT', 'REST'],
    key: '{color.decorative.neutral.DEFAULT.REST}',
  },
  'hpe.color.decorative.neutral.hover': {
    $description: 'Highest emphasis variant of neutral palette.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#d4d8db',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Highest emphasis variant of neutral palette.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.400}',
    },
    name: 'hpe.color.decorative.neutral.hover',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'neutral',
      subitem: 'DEFAULT',
      state: 'hover',
    },
    path: ['color', 'decorative', 'neutral', 'DEFAULT', 'hover'],
    key: '{color.decorative.neutral.DEFAULT.hover}',
  },
  'hpe.color.decorative.purple': {
    $description:
      'Use for decorative purposes when color has no specific meaning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: '#7764fc',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for decorative purposes when color has no specific meaning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '{base.color.purple.400}',
    },
    name: 'hpe.color.decorative.purple',
    attributes: {
      category: 'color',
      type: 'decorative',
      item: 'purple',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'decorative', 'purple', 'DEFAULT', 'REST'],
    key: '{color.decorative.purple.DEFAULT.REST}',
  },
  'hpe.color.focus': {
    $description:
      'The primary outer ring of the focus indicator, used to define the overall area of the focused element.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'The primary outer ring of the focus indicator, used to define the overall area of the focused element.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.100}',
    },
    name: 'hpe.color.focus',
    attributes: {
      category: 'color',
      type: 'focus',
      item: 'DEFAULT',
      subitem: 'REST',
    },
    path: ['color', 'focus', 'DEFAULT', 'REST'],
    key: '{color.focus.DEFAULT.REST}',
  },
  'hpe.color.focus.support': {
    $description:
      'The secondary inner ring of the focus indicator, designed to provide a high-contrast complement to color.focus against various backgrounds.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'The secondary inner ring of the focus indicator, designed to provide a high-contrast complement to color.focus against various backgrounds.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.focus.support',
    attributes: {
      category: 'color',
      type: 'focus',
      item: 'support',
    },
    path: ['color', 'focus', 'support'],
    key: '{color.focus.support}',
  },
  'hpe.color.foreground.critical': {
    $description:
      'Use for foreground regions communicating error or danger. Often used to color values in a meter or progress bar. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#fc6161',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for foreground regions communicating error or danger. Often used to color values in a meter or progress bar. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.red.500}',
    },
    name: 'hpe.color.foreground.critical',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'critical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'critical', 'DEFAULT', 'REST'],
    key: '{color.foreground.critical.DEFAULT.REST}',
  },
  'hpe.color.foreground.info': {
    $description:
      'Use for foreground regions representing a known, non-urgent status, such as informational-severity alerts or values that are noted but require no action. Often used in meters, progress bars, or chart segments where the value does not indicate a problem. Keywords: status, informational, low severity',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR', 'EFFECT_COLOR'],
      },
    },
    $type: 'color',
    $value: '#65aef9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for foreground regions representing a known, non-urgent status, such as informational-severity alerts or values that are noted but require no action. Often used in meters, progress bars, or chart segments where the value does not indicate a problem. Keywords: status, informational, low severity',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR', 'EFFECT_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.200}',
    },
    name: 'hpe.color.foreground.info',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'info',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'info', 'DEFAULT', 'REST'],
    key: '{color.foreground.info.DEFAULT.REST}',
  },
  'hpe.color.foreground.ok': {
    $description:
      'Use for foreground regions communicating success, completion, or a normal/ok status. Often used to color values in a meter or progress bar. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for foreground regions communicating success, completion, or a normal/ok status. Often used to color values in a meter or progress bar. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.foreground.ok',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'ok',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'ok', 'DEFAULT', 'REST'],
    key: '{color.foreground.ok.DEFAULT.REST}',
  },
  'hpe.color.foreground.primary': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.foreground.primary',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'primary',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'primary', 'DEFAULT', 'REST'],
    key: '{color.foreground.primary.DEFAULT.REST}',
  },
  'hpe.color.foreground.unknown': {
    $description:
      'Use for foreground regions communicating unknown status. Often used to color values in a meter or progress bar. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#7d8a92',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for foreground regions communicating unknown status. Often used to color values in a meter or progress bar. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.600}',
    },
    name: 'hpe.color.foreground.unknown',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'unknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'unknown', 'DEFAULT', 'REST'],
    key: '{color.foreground.unknown.DEFAULT.REST}',
  },
  'hpe.color.foreground.warning': {
    $description:
      'Use for foreground regions communicating warning or caution. Often used to color values in a meter or progress bar. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#ec8c25',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for foreground regions communicating warning or caution. Often used to color values in a meter or progress bar. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.orange.500}',
    },
    name: 'hpe.color.foreground.warning',
    attributes: {
      category: 'color',
      type: 'foreground',
      item: 'warning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'foreground', 'warning', 'DEFAULT', 'REST'],
    key: '{color.foreground.warning.DEFAULT.REST}',
  },
  'hpe.color.icon.critical': {
    $description:
      'Use for icons communicating errors or danger. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#fc6161',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for icons communicating errors or danger. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.foreground.critical.DEFAULT.REST}',
    },
    name: 'hpe.color.icon.critical',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'critical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'critical', 'DEFAULT', 'REST'],
    key: '{color.icon.critical.DEFAULT.REST}',
  },
  'hpe.color.icon.default': {
    $description:
      'Default icon color that is accessible on standard background colors.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Default icon color that is accessible on standard background colors.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.icon.default',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'default',
      subitem: 'REST',
    },
    path: ['color', 'icon', 'default', 'REST'],
    key: '{color.icon.default.REST}',
  },
  'hpe.color.icon.disabled': {
    $description:
      'Disabled color for icons. Using disabled colors ensures that disabled components are consistently styled.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.24)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Disabled color for icons. Using disabled colors ensures that disabled components are consistently styled.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.disabled.DEFAULT.REST}',
    },
    name: 'hpe.color.icon.disabled',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'disabled',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'disabled', 'DEFAULT', 'REST'],
    key: '{color.icon.disabled.DEFAULT.REST}',
  },
  'hpe.color.icon.info': {
    $description:
      'Use for icons communicating neutral information. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#65aef9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for icons communicating neutral information. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.blue.200}',
    },
    name: 'hpe.color.icon.info',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'info',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'info', 'DEFAULT', 'REST'],
    key: '{color.icon.info.DEFAULT.REST}',
  },
  'hpe.color.icon.ok': {
    $description: 'Use for icons communicating success. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Use for icons communicating success. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.icon.ok',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'ok',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'ok', 'DEFAULT', 'REST'],
    key: '{color.icon.ok.DEFAULT.REST}',
  },
  'hpe.color.icon.onPrimaryStrong': {
    $description:
      'Icon color to be used when icon sits on primary background. This icon and background pairing ensures accessible contrast ratios.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Icon color to be used when icon sits on primary background. This icon and background pairing ensures accessible contrast ratios.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.icon.onPrimaryStrong',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'onPrimaryStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'onPrimaryStrong', 'DEFAULT', 'REST'],
    key: '{color.icon.onPrimaryStrong.DEFAULT.REST}',
  },
  'hpe.color.icon.onSelectedPrimary': {
    $description:
      'Icon color to be used for icon sitting on non-strong background-selected variants.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Icon color to be used for icon sitting on non-strong background-selected variants.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.icon.strong.REST}',
    },
    name: 'hpe.color.icon.onSelectedPrimary',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'onSelectedPrimary',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'onSelectedPrimary', 'DEFAULT', 'REST'],
    key: '{color.icon.onSelectedPrimary.DEFAULT.REST}',
  },
  'hpe.color.icon.onSelectedPrimaryStrong': {
    $description:
      'Icon color to be used for icon sitting on background-selected-strong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Icon color to be used for icon sitting on background-selected-strong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.icon.onStrong.DEFAULT.REST}',
    },
    name: 'hpe.color.icon.onSelectedPrimaryStrong',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'onSelectedPrimaryStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'onSelectedPrimaryStrong', 'DEFAULT', 'REST'],
    key: '{color.icon.onSelectedPrimaryStrong.DEFAULT.REST}',
  },
  'hpe.color.icon.onStrong': {
    $description:
      'Icon color to be used on strong backgrounds. For example, background-neutral-xstrong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'TEXT_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Icon color to be used on strong backgrounds. For example, background-neutral-xstrong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'TEXT_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.icon.onStrong',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'onStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'onStrong', 'DEFAULT', 'REST'],
    key: '{color.icon.onStrong.DEFAULT.REST}',
  },
  'hpe.color.icon.primary': {
    $description:
      'High emphasis color for icons that should draw from the primary palette. Often used to highlight that interacting with a given element will result in an action.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'High emphasis color for icons that should draw from the primary palette. Often used to highlight that interacting with a given element will result in an action.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.icon.primary',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'primary',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'primary', 'DEFAULT', 'REST'],
    key: '{color.icon.primary.DEFAULT.REST}',
  },
  'hpe.color.icon.primary.hover': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#66ebcf',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.450}',
    },
    name: 'hpe.color.icon.primary.hover',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'primary',
      subitem: 'DEFAULT',
      state: 'hover',
    },
    path: ['color', 'icon', 'primary', 'DEFAULT', 'hover'],
    key: '{color.icon.primary.DEFAULT.hover}',
  },
  'hpe.color.icon.strong': {
    $description: 'Emphasized icon color for increased visual prominence.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Emphasized icon color for increased visual prominence.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.icon.strong',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'strong',
      subitem: 'REST',
    },
    path: ['color', 'icon', 'strong', 'REST'],
    key: '{color.icon.strong.REST}',
  },
  'hpe.color.icon.unknown': {
    $description:
      'Use for icons communicating unknown status. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#7d8a92',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for icons communicating unknown status. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.600}',
    },
    name: 'hpe.color.icon.unknown',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'unknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'unknown', 'DEFAULT', 'REST'],
    key: '{color.icon.unknown.DEFAULT.REST}',
  },
  'hpe.color.icon.warning': {
    $description:
      'Use for icons communicating warning or caution. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#ec8c25',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for icons communicating warning or caution. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{base.color.orange.500}',
    },
    name: 'hpe.color.icon.warning',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'warning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'icon', 'warning', 'DEFAULT', 'REST'],
    key: '{color.icon.warning.DEFAULT.REST}',
  },
  'hpe.color.icon.weak': {
    $description: 'Subtle icon color for reduced visual prominence.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#b1b9be',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Subtle icon color for reduced visual prominence.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['FRAME_FILL', 'SHAPE_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.weak.REST}',
    },
    name: 'hpe.color.icon.weak',
    attributes: {
      category: 'color',
      type: 'icon',
      item: 'weak',
      subitem: 'REST',
    },
    path: ['color', 'icon', 'weak', 'REST'],
    key: '{color.icon.weak.REST}',
  },
  'hpe.color.text.anchor': {
    $description: 'Text color for anchors.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color for anchors.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.primary.DEFAULT.REST}',
    },
    name: 'hpe.color.text.anchor',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'anchor',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'anchor', 'DEFAULT', 'REST'],
    key: '{color.text.anchor.DEFAULT.REST}',
  },
  'hpe.color.text.anchor.hover': {
    $description: 'Text color for anchors on hover.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL', 'STROKE_COLOR'],
      },
    },
    $type: 'color',
    $value: '#00e0af',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color for anchors on hover.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL', 'STROKE_COLOR'],
        },
      },
      $type: 'color',
      $value: '{color.text.primary.DEFAULT.hover}',
    },
    name: 'hpe.color.text.anchor.hover',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'anchor',
      subitem: 'DEFAULT',
      state: 'hover',
    },
    path: ['color', 'text', 'anchor', 'DEFAULT', 'hover'],
    key: '{color.text.anchor.DEFAULT.hover}',
  },
  'hpe.color.text.critical': {
    $description:
      'Use for text on standard backgrounds communicating errors or danger. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ff7b7b',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for text on standard backgrounds communicating errors or danger. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.red.400}',
    },
    name: 'hpe.color.text.critical',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'critical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'critical', 'DEFAULT', 'REST'],
    key: '{color.text.critical.DEFAULT.REST}',
  },
  'hpe.color.text.default': {
    $description:
      'Default text color that is accessible on standard background colors.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Default text color that is accessible on standard background colors.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.200}',
    },
    name: 'hpe.color.text.default',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'default',
      subitem: 'REST',
    },
    path: ['color', 'text', 'default', 'REST'],
    key: '{color.text.default.REST}',
  },
  'hpe.color.text.disabled': {
    $description:
      'Disabled color for text. Using disabled colors ensures that disabled components are consistently styled.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: 'rgba(255, 255, 255, 0.24)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Disabled color for text. Using disabled colors ensures that disabled components are consistently styled.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.opacity24}',
    },
    name: 'hpe.color.text.disabled',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'disabled',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'disabled', 'DEFAULT', 'REST'],
    key: '{color.text.disabled.DEFAULT.REST}',
  },
  'hpe.color.text.heading': {
    $description: 'Text color for headings.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color for headings.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.heading',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'heading',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'heading', 'DEFAULT', 'REST'],
    key: '{color.text.heading.DEFAULT.REST}',
  },
  'hpe.color.text.info': {
    $description:
      'Use for text on standard backgrounds communicating neutral information. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for text on standard backgrounds communicating neutral information. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.info',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'info',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'info', 'DEFAULT', 'REST'],
    key: '{color.text.info.DEFAULT.REST}',
  },
  'hpe.color.text.ok': {
    $description:
      'Use for text on standard backgrounds communicating success. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for text on standard backgrounds communicating success. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.ok',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'ok',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'ok', 'DEFAULT', 'REST'],
    key: '{color.text.ok.DEFAULT.REST}',
  },
  'hpe.color.text.onCritical': {
    $description: 'Text color to be used on background-critical.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color to be used on background-critical.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.onCritical',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onCritical',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onCritical', 'DEFAULT', 'REST'],
    key: '{color.text.onCritical.DEFAULT.REST}',
  },
  'hpe.color.text.onCritical.strong': {
    $description:
      'Emphasized variant of text color to be used on background-critical.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Emphasized variant of text color to be used on background-critical.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onCritical.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onCritical',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'text', 'onCritical', 'strong', 'REST'],
    key: '{color.text.onCritical.strong.REST}',
  },
  'hpe.color.text.onInfo': {
    $description: 'Text color to be used on background-info.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color to be used on background-info.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.onInfo',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onInfo',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onInfo', 'DEFAULT', 'REST'],
    key: '{color.text.onInfo.DEFAULT.REST}',
  },
  'hpe.color.text.onInfo.strong': {
    $description:
      'Emphasized variant of text color to be used on background-info.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Emphasized variant of text color to be used on background-info.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onInfo.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onInfo',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'text', 'onInfo', 'strong', 'REST'],
    key: '{color.text.onInfo.strong.REST}',
  },
  'hpe.color.text.onOk': {
    $description: 'Text color to be used on background-ok.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color to be used on background-ok.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.onOk',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onOk',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onOk', 'DEFAULT', 'REST'],
    key: '{color.text.onOk.DEFAULT.REST}',
  },
  'hpe.color.text.onOk.strong': {
    $description:
      'Emphasized variant of text color to be used on background-ok.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Emphasized variant of text color to be used on background-ok.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onOk.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onOk',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'text', 'onOk', 'strong', 'REST'],
    key: '{color.text.onOk.strong.REST}',
  },
  'hpe.color.text.onPrimaryStrong': {
    $description:
      'Text color to be used for text sitting on a primary background. This text and background pairing helps ensure accessibility.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Text color to be used for text sitting on a primary background. This text and background pairing helps ensure accessibility.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.text.onPrimaryStrong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onPrimaryStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onPrimaryStrong', 'DEFAULT', 'REST'],
    key: '{color.text.onPrimaryStrong.DEFAULT.REST}',
  },
  'hpe.color.text.onSelectedPrimary': {
    $description:
      'Text color to be used for text sitting on non-strong variants of background-selected.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Text color to be used for text sitting on non-strong variants of background-selected.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onSelectedPrimary',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onSelectedPrimary',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onSelectedPrimary', 'DEFAULT', 'REST'],
    key: '{color.text.onSelectedPrimary.DEFAULT.REST}',
  },
  'hpe.color.text.onSelectedPrimaryStrong': {
    $description:
      'Text color to be used for text sitting on background-selected-strong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Text color to be used for text sitting on background-selected-strong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.onStrong.DEFAULT.REST}',
    },
    name: 'hpe.color.text.onSelectedPrimaryStrong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onSelectedPrimaryStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onSelectedPrimaryStrong', 'DEFAULT', 'REST'],
    key: '{color.text.onSelectedPrimaryStrong.DEFAULT.REST}',
  },
  'hpe.color.text.onStrong': {
    $description:
      'Text color to be used on strong backgrounds. For example, background-neutral-xstrong.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#292d3a',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Text color to be used on strong backgrounds. For example, background-neutral-xstrong.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.1000}',
    },
    name: 'hpe.color.text.onStrong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onStrong',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onStrong', 'DEFAULT', 'REST'],
    key: '{color.text.onStrong.DEFAULT.REST}',
  },
  'hpe.color.text.onUnknown': {
    $description: 'Text color to be used on background-unknown.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color to be used on background-unknown.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.onUnknown',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onUnknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onUnknown', 'DEFAULT', 'REST'],
    key: '{color.text.onUnknown.DEFAULT.REST}',
  },
  'hpe.color.text.onUnknown.strong': {
    $description:
      'Emphasized variant of text color to be used on background-unknown.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Emphasized variant of text color to be used on background-unknown.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onUnknown.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onUnknown',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'text', 'onUnknown', 'strong', 'REST'],
    key: '{color.text.onUnknown.strong.REST}',
  },
  'hpe.color.text.onWarning': {
    $description: 'Text color to be used on background-warning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Text color to be used on background-warning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.onWarning',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onWarning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'onWarning', 'DEFAULT', 'REST'],
    key: '{color.text.onWarning.DEFAULT.REST}',
  },
  'hpe.color.text.onWarning.strong': {
    $description:
      'Emphasized variant of text color to be used on background-warning.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Emphasized variant of text color to be used on background-warning.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.strong.REST}',
    },
    name: 'hpe.color.text.onWarning.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'onWarning',
      subitem: 'strong',
      state: 'REST',
    },
    path: ['color', 'text', 'onWarning', 'strong', 'REST'],
    key: '{color.text.onWarning.strong.REST}',
  },
  'hpe.color.text.placeholder': {
    $description: 'Placeholder text color.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#b1b9be',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Placeholder text color.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.weak.REST}',
    },
    name: 'hpe.color.text.placeholder',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'placeholder',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'placeholder', 'DEFAULT', 'REST'],
    key: '{color.text.placeholder.DEFAULT.REST}',
  },
  'hpe.color.text.primary': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#05cc93',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.550}',
    },
    name: 'hpe.color.text.primary',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'primary',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'primary', 'DEFAULT', 'REST'],
    key: '{color.text.primary.DEFAULT.REST}',
  },
  'hpe.color.text.primary.hover': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#00e0af',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.green.500}',
    },
    name: 'hpe.color.text.primary.hover',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'primary',
      subitem: 'DEFAULT',
      state: 'hover',
    },
    path: ['color', 'text', 'primary', 'DEFAULT', 'hover'],
    key: '{color.text.primary.DEFAULT.hover}',
  },
  'hpe.color.text.strong': {
    $description: 'Emphasized text color for increased visual prominence.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['SHAPE_FILL', 'TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#ffffff',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: 'Emphasized text color for increased visual prominence.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['SHAPE_FILL', 'TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.white.100}',
    },
    name: 'hpe.color.text.strong',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'strong',
      subitem: 'REST',
    },
    path: ['color', 'text', 'strong', 'REST'],
    key: '{color.text.strong.REST}',
  },
  'hpe.color.text.unknown': {
    $description:
      'Use for text on standard backgrounds communicating an unknown status. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for text on standard backgrounds communicating an unknown status. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.unknown',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'unknown',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'unknown', 'DEFAULT', 'REST'],
    key: '{color.text.unknown.DEFAULT.REST}',
  },
  'hpe.color.text.warning': {
    $description:
      'Use for text on standard backgrounds communicating warning or caution. Keywords: status',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#e6e8e9',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Use for text on standard backgrounds communicating warning or caution. Keywords: status',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{color.text.default.REST}',
    },
    name: 'hpe.color.text.warning',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'warning',
      subitem: 'DEFAULT',
      state: 'REST',
    },
    path: ['color', 'text', 'warning', 'DEFAULT', 'REST'],
    key: '{color.text.warning.DEFAULT.REST}',
  },
  'hpe.color.text.weak': {
    $description:
      'Subtle text color for reduced visual prominence. Often used for supporting text.',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['TEXT_FILL'],
      },
    },
    $type: 'color',
    $value: '#b1b9be',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description:
        'Subtle text color for reduced visual prominence. Often used for supporting text.',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['TEXT_FILL'],
        },
      },
      $type: 'color',
      $value: '{base.color.grey.500}',
    },
    name: 'hpe.color.text.weak',
    attributes: {
      category: 'color',
      type: 'text',
      item: 'weak',
      subitem: 'REST',
    },
    path: ['color', 'text', 'weak', 'REST'],
    key: '{color.text.weak.REST}',
  },
  'hpe.color.transparent': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'color',
    $value: 'rgba(0, 0, 0, 0)',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'color',
      $value: '#00000000',
    },
    name: 'hpe.color.transparent',
    attributes: {
      category: 'color',
      type: 'transparent',
    },
    path: ['color', 'transparent'],
    key: '{color.transparent}',
  },
  'hpe.shadow.large': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'shadow',
    $value: '0px 12px 24px 0px rgba(0, 0, 0, 0.48) ',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'shadow',
      $value: [
        {
          blur: '{static.spacing.medium}',
          color: '{base.color.black.opacity48}',
          offsetX: '{static.spacing.none}',
          offsetY: '{static.spacing.xsmall}',
          spread: '{static.spacing.none}',
        },
      ],
    },
    name: 'hpe.shadow.large',
    attributes: {
      category: 'shadow',
      type: 'large',
    },
    path: ['shadow', 'large'],
    key: '{shadow.large}',
  },
  'hpe.shadow.medium': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'shadow',
    $value: '0px 6px 12px 0px rgba(0, 0, 0, 0.36) ',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'shadow',
      $value: [
        {
          blur: '{static.spacing.xsmall}',
          color: '{base.color.black.opacity36}',
          offsetX: '{static.spacing.none}',
          offsetY: '{static.spacing.3xsmall}',
          spread: '{static.spacing.none}',
        },
      ],
    },
    name: 'hpe.shadow.medium',
    attributes: {
      category: 'shadow',
      type: 'medium',
    },
    path: ['shadow', 'medium'],
    key: '{shadow.medium}',
  },
  'hpe.shadow.small': {
    $description: '',
    $extensions: {
      'com.figma': {
        codeSyntax: {},
        hiddenFromPublishing: false,
        scopes: ['ALL_SCOPES'],
      },
    },
    $type: 'shadow',
    $value: '0 2px 4px rgba(0, 0, 0, 0.24) ',
    filePath: 'tokens/semantic/color.dark.json',
    isSource: true,
    original: {
      $description: '',
      $extensions: {
        'com.figma': {
          codeSyntax: {},
          hiddenFromPublishing: false,
          scopes: ['ALL_SCOPES'],
        },
      },
      $type: 'shadow',
      $value: [
        {
          blur: '{base.dimension.100}',
          color: '{base.color.black.opacity24}',
          offsetX: 0,
          offsetY: '{base.dimension.50}',
          spread: 0,
        },
      ],
    },
    name: 'hpe.shadow.small',
    attributes: {
      category: 'shadow',
      type: 'small',
    },
    path: ['shadow', 'small'],
    key: '{shadow.small}',
  },
};
